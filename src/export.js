(function (root, factory) {
  ...
  function downloadSubmissionPdf(state, content, JsPdfConstructor) {
    ...
    function footer() {
      ...
      pdf.text('Marcus Aurelius: Emperor, Stoic, Citizen', page.left, page.height - 23);
    ...
    pdf.setFontSize(15);
    pdf.setFont('helvetica', 'normal');
    pdf.text('Emperor, Stoic, Citizen', page.left, 108);
    ...
    for (const question of document.questions) {
      ...
      if (question.evidence.length) {
        ...
        for (const evidence of question.evidence) {
          paragraph(`“${evidence.text}” (${evidence.citation})`, { style: 'italic', size: 9.5, indent: 12, color: [72, 72, 78], after: 6 });
        }
      }
      y += 10;
    }
