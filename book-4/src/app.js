(function () {
  ...
  function topbar() {
    ...
    return `
      <header class="topbar">
        <div class="brand-small">Marcus Aurelius<br>Meditations · Book IV</div>
      ...
      <div class="app-shell">
        <section class="title-screen">
          <div class="title-copy">
            ...
            <h1 class="display-title">Marcus Aurelius:<br>Meditations,<br>Book IV</h1>
            <p class="title-subtitle">Read one book closely: test its arguments about judgment, mortality, citizenship, reputation, and the common good.</p>
            ...
            <div class="title-actions">
              ...
              <a class="button button--ghost" href="../Marcus Aurelius.pdf#page=112" target="_blank" rel="noopener">Open Book IV in scan</a>
            </div>
          ...
          ${visualField('title', 'assets/book4-title-meditations.png')}
        </section>
        ${footer()}
      </div>`;
  }
    ...
    if (step === 0) {
      ...
      example = `
        <div class="onboarding-example">
          ...
          <p id="practice-source" class="practice-source">If so, we are citizens. If so, we are fellow-members of an organised community.</p>
    ...
    return `
      <section class="source-overlay" role="dialog" aria-modal="true" aria-labelledby="source-title">
        <header class="source-header">
          ...
          <p>Complete Book IV from C. R. Haines’s 1916 translation · text checked against the local scan</p>
        ...
        <div class="source-grid">
          <div class="source-browser ${sourceActive ? 'is-active' : ''}">
            <div class="source-tools">
              <input id="source-search" type="search" value="${escapeHtml(sourceQuery)}" placeholder="Search all of Book IV" aria-label="Search Book IV">
              <select id="source-book" aria-label="Filter by book">
                <option value="IV" selected>Book IV</option>
              </select>
            </div>
            <div class="source-results">
              ${results.length ? results.map((passage) => `
                ...
                </article>`).join('') : '<p>No passages match this search. Try a shorter or different term.</p>'}
            </div>
          </div>
  ...
  function handleAction(action, target) {
    if (action === 'start') {
      ...
      if (selected) { result.value = `“${selected}” (IV.4)`; announce('Practice quotation inserted.'); }
